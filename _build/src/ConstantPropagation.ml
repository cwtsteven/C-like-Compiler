...
let rec optimise_unary_op (op, e) v_table can_progress : expr = 
	match op with
	| Print 	-> UnaryOp(Print, optimise_expr e v_table can_progress)
...
let optimise_declare_stmnt declare_stmnt v_table can_progress : declare_stmnt = 
...
let rec optimise_block block v_table can_progress : block = 
	...
	for i = 0 to (List.length block) - 1 do 
		...
		block_array.(i) <- (
		match stmnt with
		| Expr e 					-> 	if !can_progress then Expr (optimise_expr e v_table can_progress) else Expr e
		| Return e 					-> 	if !can_progress then Return (optimise_expr e v_table can_progress) else Return e 
		| Local s 					-> 	if !can_progress then Local (optimise_declare_stmnt s v_table can_progress) else Local s
		| If_Then_Else (e, b1, b2)	-> 	let v_table' : (var, expr option) Hashtbl.t = Hashtbl.create 10
										...
										) else If_Then_Else (e, optimise_block b1 v_table' can_progress', optimise_block b2 v_table' can_progress')
		| While (e, b) 				-> 	let v_table' : (var, expr option) Hashtbl.t = Hashtbl.create 10
										and can_progress' = ref true in
										if !can_progress then (can_progress := false; While (optimise_expr e v_table can_progress, optimise_block b v_table' can_progress')) else While (e, optimise_block b v_table' can_progress')
...
let optimise program : program = 
	...
	for i = 0 to (List.length program) - 1 do 
		...
		program_array.(i) <- (
		match declare_stmnt with
		| Global s  				->	if !can_progress then Global (optimise_declare_stmnt s v_table can_progress) else Global s
