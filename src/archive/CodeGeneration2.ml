...
let rec generate_block block v_tables offset : string = 
	...
	| ((Block b) :: bs) 							-> 	let v_table : ((var, (type_ * int)) Hashtbl.t) ref = ref (Hashtbl.create 10) in
														let v_tables = v_table :: v_tables in
														let b' = generate_block b v_tables offset in 
...
let generate program : string = 
	let v_tables : (((var, (type_ * int)) Hashtbl.t) ref) list = []
	and v_table : ((var, (type_ * int)) Hashtbl.t) ref = ref (Hashtbl.create 10) 
